...
void start_process(void* filename_) { 
	...
	put_str("start_process get_a_page\n");
    //proc_stack->esp = (void*)((uint32_t)get_a_page(PF_USER,USER_STACK3_VADDR) + PG_SIZE) ; 
	uint32_t temp=(uint32_t)get_a_page(PF_USER,USER_STACK3_VADDR);
	put_str("start_process get_a_page temp\n");
	temp+=PG_SIZE;
put_str("start_process +temp\n");
	proc_stack->esp=(void*)temp;
put_str("start_process =temp\n");
	put_str("start_process OOOOOOOOOOOOOOOO\n");
