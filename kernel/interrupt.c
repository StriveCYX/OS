...
extern intr_handler intr_entry_table[IDT_DESC_CNT];	// 声明引用定义在 kernel.S中的中断处理函数入口数组
													// 
/* 创建中断门描述符 */
static void make_idt_desc(struct gate_desc* p_gdesc, uint8_t attr, intr_handler function) {
...
static void general_intr_handler(uint8_t vec_nr) {
	...
	put_str("num: ");put_int(vec_nr);put_str("\n");
	set_cursor(88);
	// 从第 2 行第 8 个字符开始打印
...
enum intr_status intr_get_status() {
	uint32_t eflags = 0;
	GET_EFLAGS(eflags);	//本文件的#define定义的宏函数
